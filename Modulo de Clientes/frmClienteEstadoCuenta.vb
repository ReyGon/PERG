...
Public Class frmClienteEstadoCuenta
    ...
    Private Sub fnLlenarEstado1()
        ...
        Using conn As EntityConnection = New EntityConnection(mdlPublicVars.entityBuilder.ToString)
            ...
            conexion.CommandTimeout = 1000000
            ...
            Try
                ...
                Dim saldoInicial = From x In conexion.sp_saldoInicial(cliente, fechaInicial)
                ...
                For Each sp As sp_SaldoPendiente_Result In saldoPendiente
                    lblSaldoVencido.Text = Format(sp.SaldoVencido, mdlPublicVars.formatoMoneda)
    ...
    Private Sub fnEstableceFinal_Estado1()
        Try
            ''RadMessageBox.Show("Si Entro aqui", nombreSistema, MessageBoxButtons.OK, RadMessageIcon.Exclamation)
        ...
        Catch ex As Exception
            ...
            ''RadMessageBox.Show("Fallo", nombreSistema, MessageBoxButtons.OK, RadMessageIcon.Exclamation)
        End Try
    End Sub
    ...
    Private Sub fnSalir() Handles Me.panel3
        Me.Close()
    End Sub
    Private Sub fnajustar() Handles Me.panel4
        If RadMessageBox.Show("Desea Ajustar todos los saldos de los clientes? ", mdlPublicVars.nombreSistema, MessageBoxButtons.YesNo, RadMessageIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Try
                Dim conexion As New dsi_pos_demoEntities
                Using conn As EntityConnection = New EntityConnection(mdlPublicVars.entityBuilder.ToString)
                    conn.Open()
                    conexion = New dsi_pos_demoEntities(mdlPublicVars.entityBuilder.ConnectionString)
                        ...
                        Dim cons = Nothing
                    cons = conexion.sp_herramientas_clienteSaldos
                    RadMessageBox.Show("Saldos Ajustados", mdlPublicVars.nombreSistema, MessageBoxButtons.OK, RadMessageIcon.Info)
                        conn.Close()

                End Using
            Catch ex As Exception
                MessageBox.Show("Error al ajustar saldos " + ex.ToString())
            End Try
        End If
    End Sub
        ...
        Try
            ...
            Using conn As EntityConnection = New EntityConnection(mdlPublicVars.entityBuilder.ToString)
                ...
                If cmbCliente.SelectedValue = Nothing Then
                    RadMessageBox.Show("Necesita escojer un Cliente", mdlPublicVars.nombreSistema, MessageBoxButtons.OK, RadMessageIcon.Exclamation)

                Else
                    idcliente = cmbCliente.SelectedValue
                    Dim cons = Nothing
                    cons = conexion.sp_herramientas_clienteSaldosIndividual(idcliente)
