...
Public Class frmBancoDebitoLista
    Public filtroActivo As Boolean
    Private permiso As New clsPermisoUsuario

    Private Sub frmBancoDebitoLista_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
    ...
    Private Sub llenagrid()
        Try
            ...
            Else
                'DATOS
                Dim consulta = (From x In ctx.tblBanco_Debitos
                                Select x.codigo, Fecha = x.fechaRegistro, Beneficiario = x.nombre,
                                Documento = x.documento, Correlativo = x.correlativo, Total = x.monto,
    ...
    Private Sub frm_nuevo() Handles Me.nuevoRegistro
        fnCambioFila()
        Try
            frmBancoDebito.Text = "Débito"
            frmBancoDebito.StartPosition = FormStartPosition.CenterScreen
            frmBancoDebito.bitDebito = True
            frmBancoDebito.ShowDialog()
            frmBancoDebito.Dispose()
        Catch ex As Exception
            alertas.fnError()
        End Try
    End Sub
    ...
    Private Sub frm_eliminar() Handles Me.eliminaRegistro
        Try
            If Me.grdDatos.RowCount > 0 Then
                If RadMessageBox.Show("¿Desea anular el débito?", mdlPublicVars.nombreSistema, MessageBoxButtons.YesNo, RadMessageIcon.Question) = Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub frm_ver() Handles Me.verRegistro
        ...
        If Me.grdDatos.RowCount > 0 Then
            If Me.grdDatos.CurrentRow.Index >= 0 Then
                frmBancoDebitoConcepto.Text = "Débito"
                frmBancoDebitoConcepto.idMovimiento = mdlPublicVars.superSearchId
                frmBancoDebitoConcepto.bitDebito = True
                frmBancoDebitoConcepto.bitCredito = False
                frmBancoDebitoConcepto.StartPosition = FormStartPosition.CenterScreen
                frmBancoDebitoConcepto.ShowDialog()
                frmBancoDebitoConcepto.Dispose()
            End If
        End If
    End Sub
    ...
    Private Sub fnAnula(ByVal idDebito As Integer)
        ...
        Using transaction As New TransactionScope
            Try
                ...
                Dim debito As tblBanco_Debitos = (From x In ctx.tblBanco_Debitos Where x.codigo = idDebito Select x).FirstOrDefault

                debito.fechaAnulado = fechaServer
                debito.usuarioAnula = mdlPublicVars.idUsuario
    ...
    Private Sub fnDocSalida() Handles Me.imprimir
        frmDocumentosSalida.txtTitulo.Text = "Lista : Movimiento de Débitos"
    ...
    Private Sub fnGrid_valueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdDatos.Click
        ...
        If Me.grdDatos.Rows.Count > 0 Then
            ...
            If (Me.grdDatos.CurrentColumn.Name = "chmConfirmar") And fila >= 0 Then
                ...
                If estado = 0 Then
                    'ANULADO
                    RadMessageBox.Show("El débito ya ha sido anulado", mdlPublicVars.nombreSistema, MessageBoxButtons.OK, RadMessageIcon.Exclamation)
                ElseIf estado = 1 Then
                    'POR CONFIRMAR
                    If RadMessageBox.Show("¿Desea confirmar el débito?", mdlPublicVars.nombreSistema, MessageBoxButtons.YesNo, RadMessageIcon.Question) = Windows.Forms.DialogResult.Yes Then
                        If fnConfirmarDebito(Me.grdDatos.Rows(fila).Cells("codigo").Value) Then
                            Me.frm_llenarLista()
                        Else
                ...
                ElseIf estado = 4 Then
                    'CONFIMADO
                    RadMessageBox.Show("El débito ya ha sido confirmado", mdlPublicVars.nombreSistema, MessageBoxButtons.OK, RadMessageIcon.Exclamation)
                End If
            End If
        End If
    End Sub
    ...
    Private Function fnConfirmarDebito(ByVal idDebito As Integer) As Boolean
        Dim success As Boolean = True
        Dim fechaServer As DateTime = mdlPublicVars.fnFecha_horaServidor
        ...
        Using transaction As New TransactionScope
            ...
            Try
                ...
                Dim movimiento As tblBanco_Debitos = (From x In ctx.tblBanco_Debitos Where x.codigo = idDebito _
                ...
                Else
                    ...
                    cuenta.pagosTransito -= movimiento.monto
