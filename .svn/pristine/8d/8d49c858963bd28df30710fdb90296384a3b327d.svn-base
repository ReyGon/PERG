...
Public Class frmBuscarArticuloCatalogo
    ...
    Private Sub fnLlenarCombo()
        ...
        Dim catalogos = (From x In ctx.tblArticulo_Catalogo Where x.articulo = codigo _
                            Select Codigo = x.imagen, Nombre = x.numero & " - " & x.observacion)
    ...
    Public Sub fnLlenarDatos(ByVal catalogo As String)
        Try
            ...
            If catalogo IsNot Nothing Then
                cargaFoto(catalogo)
                Dim cons = ctx.sp_buscar_Articulo(mdlPublicVars.idEmpresa, catalogo, mdlPublicVars.General_idTipoInventario, mdlPublicVars.General_idAlmacenPrincipal, "", "", codClie, 3, True, False, "", venta)

                grdInformacion.DataSource = cons 'mdlPublicVars.EntitiToDataTable(cons)
    ...
    Private Sub fnConfiguracion()
        Try
            ...
            Me.grdInformacion.Columns("UbicacionEstanteria").IsVisible = False 'Ubicacion en estanteria

            
            Me.grdInformacion.Columns(0).Width = 60 ' agregar
            Me.grdInformacion.Columns(2).Width = 75 ' codigo
            Me.grdInformacion.Columns(3).Width = 200 ' nombre
            ...
            grdInformacion.Columns("numeroArt").HeaderText = "No"
    ...
    Private Sub fnAgregar_Productos()
        ...
        For index = 0 To Me.grdInformacion.Rows.Count - 1
            ...
            ' If agrega = True Then
            If Me.grdInformacion.Rows(index).Cells("txmCantidad").Value.ToString.Length > 0 Then


                cantidad = Me.grdInformacion.Rows(index).Cells("txmCantidad").Value
                surtir = Me.grdInformacion.Rows(index).Cells("txmSurtir").Value
                estado = Me.grdInformacion.Rows(index).Cells("clrEstado").Value
    ...
    'Funcion utilizada para dar formato a las celdas
    ...
    Private Sub grdInformacion_CellDoubleClick(ByVal sender As System.Object, ByVal e As Telerik.WinControls.UI.GridViewCellEventArgs) Handles grdInformacion.CellDoubleClick
        Try
    ...
    Private Sub grdInformacion_SelectionChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles grdInformacion.SelectionChanged
        Try
            ...
            Try
                ubicacion = CType(Me.grdInformacion.Rows(Me.grdInformacion.SelectedRows(0).Index).Cells("UbicacionEstanteria").Value, String)
            Catch ex As Exception
                ubicacion = ""
            End Try
    ...
    'AGREGAR IMPRESION
    Private Sub fnImprimir() Handles Me.panel2
        Try
            ...
            Dim direccion As String = Path.Combine(mdlPublicVars.BuscarArticulo_CarpetaCatalogo, (pagina))


            Try
                Dim permiso As New clsPermisoUsuario
                frmDocumentosSalida.UrlImagen = direccion
    ...
    'FILTRAR POR CATALOGO
    Private Sub fnFiltroPorCatalogo() Handles Me.panel0
        frmBusquedaArticuloCatalogoFiltro.Text = "Filtro por catalogo"
        frmBusquedaArticuloCatalogoFiltro.StartPosition = FormStartPosition.CenterScreen
        frmBusquedaArticuloCatalogoFiltro.ShowDialog()
        frmBusquedaArticuloCatalogoFiltro.Dispose()
    End Sub
