...
Public Class clsBase
    ...
    Public Function fnGrid_seleccionarEspacio(ByVal grd As RadGridView, ByVal col As Integer, ByVal e As System.Windows.Forms.KeyEventArgs, ByVal valor As Boolean)
        Try
            If Char.ConvertFromUtf32(e.KeyValue) = ChrW(Keys.Space) Then
                Dim bitActual As Boolean = CType(grd.Rows(grd.SelectedRows(0).Index).Cells(col).Value, Boolean)
                If bitActual = True Then
                    grd.Rows(grd.SelectedRows(0).Index).Cells(col).Value = False
                Else
    ...
    Private Function fnMetodosAbreviados(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        If e.KeyValue = Keys.Escape Then
            If frm.Name.ToString.ToLower = "frmmenuprincipal" Then
                If RadMessageBox.Show("Desea Cerrar el sistema !!!", mdlPublicVars.nombreSistema, MessageBoxButtons.YesNo, RadMessageIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    Application.Exit()
                End If
                Return False
                Exit Function
