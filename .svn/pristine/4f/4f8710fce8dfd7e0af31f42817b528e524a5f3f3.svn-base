...
Public Class frmBuscarArticuloSustitutos
    ...
    Private Sub fnLlenar_sustitutos(ByVal codigoBuscar As Integer)
        Try
            ...
            If contador > 0 Then
                ...
                Me.grdSustitutos.DataSource = sustitutos ' mdlPublicVars.EntitiToDataTable(sustitutos)
                mdlPublicVars.fnGrid_iconos(Me.grdSustitutos)
                fnConfiguracion()
    ...
    Private Sub fnLlenarGrid()
        Try
            ...
            Dim cons = ctx.sp_buscar_Articulo(mdlPublicVars.idEmpresa, codigoArticulo, mdlPublicVars.General_idTipoInventario, mdlPublicVars.General_idAlmacenPrincipal, "", "", codClie, 2, True, False, "", venta)

            grdSustitutos.DataSource = cons
            fnConfiguracion()
        Catch ex As Exception
    ...
    Private Sub fnConfiguracion()
        Try
            ...
            Me.grdSustitutos.Columns("UbicacionEstanteria").IsVisible = False 'Ubicacion Estanteria
    ...
    Private Sub fnAgregar_Productos()
        ...
        For index = 0 To Me.grdSustitutos.Rows.Count - 1
            ...
            ' If agrega = True Then
            If Me.grdSustitutos.Rows(index).Cells("txmCantidad").Value IsNot Nothing And IsNumeric(Me.grdSustitutos.Rows(index).Cells("txmCantidad").Value) = True Then
                ...
                If cantidad > 0 Or surtir > 0 Then
                    ...
                    estado = Me.grdSustitutos.Rows(index).Cells("clrEstado").Value
                    Try
    ...
    Private Sub grdSustitutos_SelectionChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles grdSustitutos.SelectionChanged
        Try
            ...
            Dim fila As Integer = CInt(mdlPublicVars.fnGrid_codigoFilaSeleccionada(grdSustitutos))

            Try
                observacion = CType(Me.grdSustitutos.Rows(fila).Cells("Observacion").Value, String)
            Catch ex As Exception
                observacion = ""
            End Try
            Try
                empaque = CType(Me.grdSustitutos.Rows(fila).Cells("UnidadEmpaque").Value, String)
            Catch ex As Exception
                empaque = ""
            End Try

            Try
                marca = CType(Me.grdSustitutos.Rows(fila).Cells("Marca").Value, String)
            Catch ex As Exception
                marca = ""
            End Try

            Try
                compatibilidad = CType(Me.grdSustitutos.Rows(fila).Cells("Compatibilidad").Value, String)
            Catch ex As Exception
                compatibilidad = ""
            End Try

            Try
                ubicacion = CType(Me.grdSustitutos.Rows(fila).Cells("UbicacionEstanteria").Value, String)
            Catch ex As Exception
                ubicacion = ""
            End Try
