...
Public Class frmImpuesto
    ...
    Private Sub llenagrid()
        Try
            ...
            Dim Data = From x In ctx.tblImpuestoes Select Id = x.idImpuesto, Nombre = x.nombre, Formula = x.formula
    ...
    Private Sub frm_grabaRegistro() Handles Me.grabaRegistro
        ...
        Dim m As New tblImpuesto
        Try
            m.nombre = txtNombre.Text
            m.formula = txtFormula.Text

            ctx.AddTotblImpuestoes(m)
            ctx.SaveChanges()
            alertas.fnGuardar()
    ...
    Private Sub frm_modificaRegistro() Handles Me.modificaRegistro
        Try
            ...
            Dim m As tblImpuesto = (From e1 In ctx.tblImpuestoes Where e1.idImpuesto = Me.txtId.Text Select e1).First()
            m.nombre = txtNombre.Text
            m.formula = txtFormula.Text
